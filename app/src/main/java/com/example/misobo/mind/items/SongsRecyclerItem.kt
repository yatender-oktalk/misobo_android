...
class SongsRecyclerItem(private val model: MusicResponseModel.MusicModel,val onClick:(Int)->Unit) : Item() {
    override fun bind(viewHolder: ViewHolder, position: Int) {
        ...
        if (position == 0)
            viewHolder.itemView.rootLayout.background = ContextCompat.getDrawable(viewHolder.itemView.context,R.drawable.music_gradient)
        else if (position == 1)
            viewHolder.itemView.rootLayout.background = ContextCompat.getDrawable(viewHolder.itemView.context,R.drawable.gradient_2)
